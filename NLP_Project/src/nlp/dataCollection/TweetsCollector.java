...
public class TweetsCollector {
    public static void main(String[] args) throws IOException {
        ...
        try {
        	//Before using this class remember to set the following credentials:
            ...
            {
                List<Status> statuses = twitter.getUserTimeline(userScreenName, paging);                
                for (Status status : statuses) {
                	...
                	if(!status.getText().startsWith("RT"))
                	{
                    	...
                    	{
                    		if(statusWords[j].length() > 0 && !(statusWords[j].charAt(0) == '@'))
                    		{
                    			int httpIndex;
        						if((httpIndex = statusWords[j].indexOf("http")) != -1)
                    			{
                    				statusWords[j] = statusWords[j].substring(0, httpIndex); 
                    			}
                        ...
                        System.out.println(i + " " + status.getId() + " :: " + plainTweet);                        
                    }
